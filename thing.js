...
bookingName,
created,
(SELECT accessLevel FROM Biggly.users WHERE bms_booking.booking.createdUserKey = userKey) as accessLevel, // < if no `as` value then use the `name` again.
...
HAVING bookingsKey = "123" AND created = "2019" // < we could use HAVING rather than WHERE as it allows us to be more vague.
`
